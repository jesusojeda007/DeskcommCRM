...
export function RulesTab() {
  ...
  const toggleActive = (rule: AutomationRuleRow, checked: boolean) => {
    ...
    update.mutate(
      ...
      {
        onSuccess: () =>
          toast.success(checked ? t("Automação ligada.") : t("Automação pausada.")),
        ...
        {rules.map((r) => (
          <Card key={r.id}>
            <CardHeader className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">{t(triggerLabel(r.trigger_event))}</p>
            ...
            <CardContent className="flex items-center justify-between gap-2">
              <Switch
                ...
                aria-label={t(r.is_active ? "Pausar {nome}" : "Ligar {nome}", { nome: r.name })}
              />
      ...
      <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("{nome} para de rodar imediatamente. Essa ação não pode ser desfeita.", {
                nome: deleting?.name ?? "",
              })}
            </AlertDialogDescription>
          </AlertDialogHeader>

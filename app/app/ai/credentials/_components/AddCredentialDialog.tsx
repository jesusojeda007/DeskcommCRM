    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (justCreated?.models_available != null) {
          toast.success(
            t("Validada — {n} modelos disponíveis.", { n: justCreated.models_available }),
          );
        } else if (justCreated?.validation_error) {
          toast.error(t("Validação falhou: {motivo}", { motivo: justCreated.validation_error }));
        }
      }, 3000);
      ...
      <DialogContent>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.api_key && (
              <p className="text-xs text-destructive">{t(errors.api_key)}</p>
            )}
          </div>

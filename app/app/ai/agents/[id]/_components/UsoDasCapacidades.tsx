...
function formatarData(iso: string | null, naoDisponivel: string): string {
  if (!iso) return naoDisponivel;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          <p className="text-sm">
            ...
            {t(resumo.usos === 1 ? "uso" : "usos")}{" "}
            {t("nos últimos {dias} dias", { dias: janela_em_dias })}
            {resumo.falhas > 0 ? (
              <>
                {" · "}
            ...
            {resumo.precisam_de_atencao > 0
              ? t(
                  resumo.precisam_de_atencao === 1
                    ? "{n} capacidade pede uma decisão sua."
                    : "{n} capacidades pedem uma decisão sua.",
                  { n: resumo.precisam_de_atencao },
                )
              : t("Nada pedindo decisão no momento.")}
            {versao_lida
              ? " " +
                t("O que está ligado vem da versão {versao}.", {
                  versao: t(NOME_DA_VERSAO[versao_lida.status] ?? versao_lida.status),
                })
              : null}
          </p>
        </div>
          ...
          {capacidades.map((c) => (
            ...
            >
              ...
              <div className="flex flex-wrap gap-x-4 pt-2 font-mono text-[11px] text-muted-foreground">
                ...
                <span>{t("última vez {data}", { data: formatarData(c.ultima_vez, t("nunca")) })}</span>
              </div>
            </li>
          ))}
        </ul>

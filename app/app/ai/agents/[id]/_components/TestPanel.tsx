      ...
      */}
      <details className="text-xs text-muted-foreground">
        <summary className="cursor-pointer" data-testid="teste-nao-verificado">
          {t("O teste não consegue verificar tudo ({count} verificações ficam de fora)", {
            count: g.naoAvaliados.length,
          })}
        </summary>
...
export function TestPanel({ agent, draft, published, readOnly }: Props) {
  ...
  const versionLabel =
    target.status === "published"
      ? t("v{n} (publicada)", { n: target.version_number })
      : t("v{n} (rascunho)", { n: target.version_number });
  ...
  async function handleRun() {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        toast.error(err.message ?? t("Erro: {error}", { error: err.code }));
      } else {

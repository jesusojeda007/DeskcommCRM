  ...
  return (
    ...
    >
      <div className="space-y-2">
        <Label htmlFor="name">{t("Nome do agente")}</Label>
        <Input
          id="name"
          name="name"
          value={name}
      ...
      <fieldset className="space-y-2">
        <legend className="text-sm font-medium">{t("Estilo de prompt")}</legend>
          ...
          {TEMPLATES.map((tpl) => (
            <label
              ...
              className={cn(
                ...
                template === tpl.id ? "border-primary bg-primary/5" : "hover:bg-muted/40",
              )}
            >
              <input
                type="radio"
              ...
              />
              <span>
                <span className="block text-sm font-medium">{t(tpl.title)}</span>
                <span className="block text-xs text-muted-foreground">{t(tpl.desc)}</span>
              </span>
            </label>
          ))}
        </div>
      ...
      {naoPublicado && (
        ...
        >
          <p className="text-sm font-medium">
            {t("Seu agente foi criado, mas ficou como ")}
            <strong>{t("rascunho")}</strong>
            {t(

...
export function JanelaFechadaAviso({
  ...
  const aprovados = useMemo(
    () => (data?.data.templates ?? []).filter((m) => m.status?.toUpperCase() === "APPROVED"),
    [data],
  );

  const atual = aprovados.find((m) => `${m.name}|${m.language}` === escolhido) ?? null;
    ...
    <div className="border-t border-amber-300 bg-amber-50/60 px-4 py-3 dark:border-amber-800/60 dark:bg-amber-950/30">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {aprovados.map((modelo) => (
              <option key={`${modelo.name}|${modelo.language}`} value={`${modelo.name}|${modelo.language}`}>
                {modelo.name} ({modelo.language})
                {(modelo.slots?.length ?? 0) > 0 ? ` · ${modelo.slots!.length} parâmetro(s)` : ""}
              </option>
            ))}
          </select>
      ...
      {pedeParametros && (
        ...
        <p className="mt-2 text-[11px] text-amber-900/80 dark:text-amber-200/80">
          {t("Este modelo pede {n} valor(es) e ainda não dá para preenchê-los aqui — envie por", {
            n: atual?.slots?.length ?? 0,
          })}{" "}
          <strong>{t("Conexões → Templates")}</strong>
          {t(", ou escolha um modelo sem parâmetros.")}
        </p>
      )}
    </div>
  );
